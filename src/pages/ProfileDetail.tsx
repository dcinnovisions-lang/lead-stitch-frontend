                    ...
                    <div className="flex items-start justify-between mb-6 gap-4">
                        <div className="flex items-start space-x-4 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                                <h1 className="text-3xl font-bold text-gray-900 mb-1 break-words">{profile.name || profile.fullName || `${profile.firstName || ''} ${profile.lastName || ''}`.trim() || 'Unknown'}</h1>
                                <p className="text-lg text-gray-600 break-words">{profile.title || profile.profession || profile.headline || ''}</p>
                                {(profile.company_name || profile.company) && (
                                    <p className="text-gray-500 break-words">{profile.company_name || profile.company || ''}</p>
                                )}
                            </div>
                        </div>
                        {!profile.email && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        {profile.decision_maker_role && (
                            <div className="min-w-0">
                                ...
                                <span className="inline-block px-3 py-1 rounded-lg text-sm font-medium bg-purple-100 text-purple-800 break-words whitespace-normal max-w-full">
                                    {profile.decision_maker_role}
                                </span>
                            </div>
                        )}
