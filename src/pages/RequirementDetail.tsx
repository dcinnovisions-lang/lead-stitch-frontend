...
import { getErrorMessage, formatErrorDisplay, getAllErrors } from '../utils/errorFormatter'
        ...
        setModal({
            ...
            message: (
                <div className="text-left">
                    ...
                    <div
                        className="max-h-[300px] overflow-y-auto mb-3 hide-scrollbar border border-gray-200 bg-gray-50 rounded-lg p-4"
                        style={{
                            scrollbarWidth: 'none',
                    ...
                    >
                        ...
                        `}</style>
                        <ul className="list-decimal list-inside space-y-1 text-gray-700 pr-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                                ...
                                {journeySteps.map((step, index) => (
                                    <div key={step.id} className="flex items-start gap-4">
                                        <div className="flex flex-col items-center">
                                            <div
                                                className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-bold ${step.complete
                                            ...
                                            {index < journeySteps.length - 1 && (
                                                <div
                                                    className={`w-px h-12 ${journeySteps[index + 1].complete ? 'bg-blue-400' : 'bg-gray-200'
                                                        }`}
                                                ></div>
                                            )}
                                        </div>
                        ...
                        <div className="bg-white border border-gray-200 rounded-3xl shadow-sm p-6">
                            ...
                            {hasDecisionMakers ? (
                                <div
                                    className="max-h-[70vh] min-h-[400px] overflow-y-auto hide-scrollbar"
                                    style={{
                                        scrollbarWidth: 'none',
                                        msOverflowStyle: 'none',
                                    } as React.CSSProperties & { msOverflowStyle?: string }}
                        ...
                        {(scrapingInProgress ||
                            (scrapingStatus?.status &&
                                ...
                                (scrapingStatus.status === 'processing' || scrapingStatus.status === 'running' || scrapingStatus.status === 'pending'))) && (() => {
                                    ...
                                    const currentStep = scrapingStatus?.current_step || 'pending'
                                    const stepDetails = scrapingStatus?.step_details || { step: 'pending', message: 'Initializing...', progress: 0 }
                                    const currentRole = scrapingStatus?.currentRole || null
                                    const profilesScraped = scrapingStatus?.profilesScraped || 0
                                    const rolesCompleted = scrapingStatus?.rolesCompleted || 0
                                    const totalRoles = scrapingStatus?.totalRoles || 0                                    
                                            ...
                                            <div className="space-y-3 mb-6 relative">
                                                {steps.map((step, index) => (
                                                    <div key={step.id} className="flex items-start gap-3 relative">
                                                        {/* Step Indicator */}
                                                        <div className="flex flex-col items-center">
                                                            <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold transition-all duration-300 ${step.completed
                                                            ...
                                                            {index < steps.length - 1 && (
                                                                <div className={`w-0.5 h-12 mt-2 transition-all duration-300 ${step.completed ? 'bg-green-400' : 'bg-white/20'
                                                                    }`}></div>
                                                            )}
                                                        </div>
                                                        ...
                                                        <div className="flex-1 pt-1">
                                                            <p className={`text-sm font-semibold mb-1 ${step.active ? 'text-white' : step.completed ? 'text-green-200' : step.failed ? 'text-red-200' : 'text-white/60'
                                                                }`}>
                                                                {step.label}
                                                                {step.failed && (
                                            ...
                                            <div className="bg-red-500/20 rounded-lg p-3 space-y-2">
                                                ...
                                                <ul className="list-disc list-inside space-y-1 text-xs text-red-100">
                                                    {allErrors.slice(1).map((error, idx) => (
                                                        <li key={idx}>{error.user_message || 'An error occurred'}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                        )}
                        ...
                        <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-3xl text-white p-6 shadow-lg">
                            ...
                            {primaryAction && primaryAction.onClick && primaryAction.cta && (
                                <button
                                    ...
                                    className={`mt-6 w-full inline-flex items-center justify-center px-4 py-3 bg-white/15 backdrop-blur rounded-2xl font-semibold transition ${primaryAction.disabled || startingScrape
                        ...
                        <div className="bg-white border border-gray-200 rounded-3xl shadow-sm p-6">
                            ...
                            <div className="mt-4 space-y-4 text-sm text-gray-700">
                                <div className="flex items-start justify-between gap-4">
                                    <span className="text-gray-500 flex-shrink-0">Industry</span>
                                    <span className="font-semibold text-right break-words">{requirement?.industry || '—'}</span>
                                </div>
                                <div className="flex items-start justify-between gap-4">
                                    <span className="text-gray-500 flex-shrink-0">Product / Service</span>
                                    <span className="font-semibold text-right break-words">{requirement?.product_service || '—'}</span>
                                </div>
                                <div className="flex items-start justify-between gap-4">
                                    <span className="text-gray-500 flex-shrink-0">Target Location</span>
                                    <span className="font-semibold text-right break-words">{requirement?.target_location || '—'}</span>
                                </div>
                                <div className="flex items-start justify-between gap-4">
                                    <span className="text-gray-500 flex-shrink-0">Target Market</span>
                                    <span className="font-semibold text-right break-words">{requirement?.target_market || '—'}</span>
                                </div>
                                <div className="flex items-start justify-between gap-4">
